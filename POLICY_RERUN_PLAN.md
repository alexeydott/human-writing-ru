...
- **Решение человека (2026-08-27): третий канал — `zenodo_livejournal_7139731`**
  ...
  это бинарный индекс, а не текстовый корпус; документных границ и тел постов
  нет, схема не задокументирована (Zenodo-запись без описания). Статус
  `catalogued_format_unverified` сохраняется; источник в зачёт diversity
  не входит (экран валидатора).
...
- **Итог 1.2: prose остаётся неготовым.** Ни один каталогизированный
  источник не закрывает все три sub-gates одновременно: для третьего канала
  нужен blog-источник с recoverable документными границами **и**
  author-provenance (как планировалось — LJ с journal-именами). Это
  отрицательный результат этапа; решения по готовым профилям (product,
...
| prose | ✅ | ❌ 2<3 источника, 2<3 канала, 43.6%<50% слов | **неготов** (zenodo — формат не верифицирован, UD Taiga — без авторов) |
...
- **R1. Сеть:** ljsear.ch и HuggingFace могут быть недоступны/урожай
  ...
  **Статус (этап 1): LJSearch — мёртв для назначения (stub-страницы, 0 из 200);
  HuggingFace — решён локальным шаром `ruslawod_11.parquet` (см. 1.3).**

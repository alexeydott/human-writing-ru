...
- **LJSearch (вариант A) — рабочий маршрут найден, тексты получены (2026-08-27).**
  ...
  документа на автора; канонический импорт `import_local_tree` сохраняет
  58: lj_37128223 (jane_volunteer) — 239 слов по WORD_RE < 250,
  lj_28057 (andrusbarabus) — russian_share 0.464 < 0.65). По `release_policy` в репозитории закреплён
...
- **Итог 1.2: prose-данные собраны и заматериализованы (2026-08-27).**
  ...
  (original-URL, см. выше): собрано 60 blog-документов с author-provenance,
  канонический импорт сохраняет 58 (lj_37128223 < 250 WORD_RE-слов,
  lj_28057 russian_share 0.464). Локальное дерево
  `data/ljsearch_saved_copies/` (60 документов + sidecar manifest.csv +
  provenance.json) подключено к пайплайну:
  `ljsearch_saved_copies.adapter = manual_or_local_tree`, автодетект
  `data/<source_id>/manifest.csv`. Прогноз: 3 источника (factrueval 80
  media + taiga 80 social + LJ 58 blog), 3 канала, авторы ≈ (80+58)/218 =
  63.3% по документам и ≈72% по словам, доля крупнейшего источника 36.7% —
  подтверждение полным прогоном `data/heldout-work-policy-1.5` (Этап 2).
  Решения по готовым профилям (product, technical, oral, official) не
  блокируются.
...
- Итог: official = pravo 50 + ruslawod 80 = 130 документов / 103 294 слова.
  ...
  (порог 80%) ✅.
- **Поправка (2026-08-27, прогон `heldout-work-policy-1.5`): gate НЕ
  закрыт — в расчёте выше арифметическая ошибка.** Фактически ruslawod
  материализован на **534 652 слова** (не ~53 462, как заложено в сумму
  103 294): official = 584 484 слова, доля ruslawod **91.5% по словам
  > 80%** (`maximum_source_word_share`, spec-v3) → diversity-гейт official
  не пройден. По документам всё в норме (38.5% ≤ 80%, источников 2).
  Цифры `target_words: 40000` у ruslawod в registry не ограничивают
  импорт (кап только по `target_documents: 80`), поэтому прогноз по
  `target_words` не отражает фактическую длину юридических актов.
  Варианты решения (за человеком): (а) принять отрицательный результат —
  решения по official-сигналам блокируются, остальные профили не
  затрагиваются; (б) разбавить pravo — нужно ≥133 663 слов pravo
  (≈134 документа при ~1000 словах) для доли ruslawod ≤ 80%;
  (в) переселекция ruslawod на более короткие акты (≤199 328 слов на 80
  документов) — требует изменения детерминированной селекции.
- **Решение реализовано (2026-08-27) — вариант (в), переселекция
  v2.** Из того же шара (4864 строки; воспроизведение правила v1
  совпало с деревом 80/80): те же eligibility (≥300 слов, ≥3 абзацев,
  russian ≥0.65, без шаблонных поправок), + **кап 4000 слов на
  документ**, порядок в ячейках — кратчайший первым (ранее —
  longest-first), та же стратификация (орган, год) и капы (8/орг,
  6/год), 80 документов. Итог: **105 517 слов, 24 органа, 29 лет,
  доля по словам 0.679** (порог 0.8, запас ~12 п.п.). Без кapa даже
  shortest-first не проходит (298 778 слов / 0.857): стратификация
  вынуждает брать длинные акты из ячеек, где коротких нет. Дерево
  пересобрано (`data/ruslawod_v3/`, provenance.json c `selection_revision:
  v2` и построчным audit-trail), registry обновлён.
...
|---|---|---|---|
| product | ✅ | ✅ | готов (разметка — в новом workspace, 45 алертов) |
| technical | ✅ | ✅ | готов |
| oral | ✅ | ✅ 72.5% / 62.4% | готов (101 алерт ждёт разметки) |
| official | ✅ | ✅ 2 источника, 38.5% / 67.9% | **готов** (подтверждено прогоном `heldout-work-policy-1.6` после переселекции v2) |
| prose | ✅ | ✅ 218 доков, 3 источника, 3 канала, авторы 63.3% / 74.3% | **готов** (подтверждено прогоном `heldout-work-policy-1.5`) |
...
**Фактический первый прогон (2026-08-27): `data/heldout-work-policy-1.5`,
exit 6 (v3 evidence не завершена — ждёт разметки).** 911 документов
материализовано (включая LJ 58). Профили: prose ✅ (218 доков, 116 136
слов, все diversity-проверки true), oral ✅, product ✅, technical ✅,
official ❌ (поправка к 1.3). Новые алерты: 171 (product 45 — корпус
product изменился: rbc 30 вместо 60, разметку beta4-final делать
непосредственно нельзя, см. R3; oral 101 — из-за расширения duma;
prose 25). `ABLATION_DECISION_V3.json`:
`ablation_v3_complete_no_automatic_policy_change`,
`natural_alert_adjudication_pending`: long-sentence/oral,
long-sentence/product, long-sentence/prose,
one-sentence-paragraphs/product.

**Второй прогон (2026-08-27) после переселекции ruslawod v2:
`data/heldout-work-policy-1.6`. Все 5 профилей `freeze=True`: official
130/160 756 (доля ruslawod по словам 0.67), prose 218/116 136, oral
255/210 017, product 167/276 736, technical 141/95 802.** 911
прошедших валидацию документов. Замечание: в этом прогоне
yandex_cloud_docs_ru не материализовался (сбой github-аквиизиции,
отчёт отсутствует) и technical временно проседал; повторный прогон
только yandex дал +80, валидация и решение пересобраны — технический
корпус по github-деревьям колеблется от прогона к прогону (kubernetes

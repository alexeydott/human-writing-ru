...
- Исправлено получение продуктовых веб-кейсов: страницы рубрик RBC исключаются, а Ruward извлекается по содержательным блокам `caseblock`, без навигационного оформления.
- Исправлен обход Yandex Cloud Docs при усечённом рекурсивном дереве GitHub API: источник теперь может обходить объявленное поддерево `ru/`.
- Обновлено руководство по профилям: описан текущий калибровочный цикл v3, обязательная ручная разметка естественных срабатываний и ограничение на смешивание каналов.
- `scripts/build_lite.py` формирует автономную краткую инструкцию для прямой вставки в сетевой диалог. Результат не требует запуска внешних средств, очищен от остаточных англицизмов и проверяется по пределу 2000 знаков, оставляющему запас относительно 2048.
- Лингвистическая политика `1.4.0` и активные пороги `editorial-baseline.json` сохранены без изменений.
